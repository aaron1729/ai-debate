...
def create_debate_plot(motion_pattern, output_filename):
    ...
    fig.suptitle(f'Debate Motion (MAX judgment IDs): {motion_text}', fontsize=13, fontweight='bold', y=0.93)
    ...
    for idx, (exp_id, claim, pro_model, con_model, pro_went_first) in enumerate(experiments):
        ...
        cursor.execute('''
            ...
            AND id IN (
                ...
                FROM judgments
                WHERE experiment_id = ?
                GROUP BY judge_model, turns_considered
            )
    ...
    note_text = ('An open circle denotes a "needs more evidence" ruling (not actually scored, but plotted here as 5).\n'
                 'Dashed lines are used to connect these rulings to others. Using MAX(id) for duplicate judgments.')
